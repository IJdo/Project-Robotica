...
import os
import sys
# sys.path.append(r"D:\School\Project-Robotica\eindcode\voicerecognition")
...
Recorder.record()
# waveform[0] = numpy.resize(waveform[0], sample_rate) # force resize to 16KHz
# waveform, sample_rate = Recorder()
# waveform, sample_rate = torchaudio.load("output.wav")
# waveform = torch.unsqueeze(waveform, 1)
# print(waveform.size())
# transformed = transform(waveform.to(device)).to(device)
# print(transformed.size())
# prediction = predict(transformed).to(device)
